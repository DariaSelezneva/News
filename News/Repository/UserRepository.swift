...
protocol UserRepositoryLogic {
    ...
    func updateUser(avatar: String, email: String, name: String) -> AnyPublisher<User, Error>
}
...
class UserRepository: UserRepositoryLogic {
    ...
    func updateUser(avatar: String, email: String, name: String) -> AnyPublisher<User, Error> {
        ...
        return AF.request(API.replaceUserURL, method: .put, parameters: parameters, encoding: JSONEncoding.default, headers: headers)
            .validate()
            .publishDecodable(type: User.self, queue: .main)
            .value()
            .mapError({$0 as Error})
